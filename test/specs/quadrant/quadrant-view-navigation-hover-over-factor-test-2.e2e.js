...
describe(`Quadrant View Navigation - Hover Over Factor - Test 2`, () => {
    ...
    it(`Verify that Grey Oval appears on mouse over`, async function () {
        let point = await brandPositioningPage.getQuadrantPointComponents("Social Impact")
        let text = point.text;

        await text.isDisplayed();
        await text.moveTo();

        let point2 = await brandPositioningPage.getQuadrantPointComponents("Social Impact");
