...
describe(`Quadrant View Navigation - Hover Over - Test 1`, () => {
    ...
    it(`Verify that a brief description of the hovered quadrant shows above the quadrant - Bottom Left`, async function(){
        let el = await brandPositioningPage.getQuadrant('bottom-left');
        await el.click()
        let desc = await brandPositioningPage.getActiveQuadrantDescription()
        assert.equal(desc, "Attributes with which the brand is unassociated", "Description in bottom-left of quadrant chart is not matching expected value.");
    })
    ...
    it(`Verify that the area description to the left of the quadrant chart updates to show a description about the hovered quadrant - Bottom Left`, async function(){
        let el = await brandPositioningPage.getQuadrant('bottom-left');
        await el.click()
        let desc = await brandPositioningPage.getQuadrantAreaDescription()
        assert.equal(desc, "Attributes with which the brand is unassociated", "Left Side Quadrant Area Description of selected quadrant (bottom-left) chart is not matching expected value.");
    })
    ...
    it(`Verify that a zoom indicator icon appears in the upper-right of the quadrant - Top Right`, async function (){
        let el = await brandPositioningPage.getQuadrant('top-right');
        await el.click()
        let expander = await brandPositioningPage.getQuadrantExpander('top-right')
        assert.equal(await expander.isDisplayed(), true, "Top Right Expander is not Visible")
    })

    it(`Verify that a zoom indicator icon appears in the upper-right of the quadrant - Bottom Left`, async function (){
        let el = await brandPositioningPage.getQuadrant('bottom-left');
        await el.click()
        let expander = await brandPositioningPage.getQuadrantExpander('bottom-left')
        assert.equal(await expander.isDisplayed(), true, "Bottom Left Expander is not Visible")
    })
    ...
    it(`Verify that clicking on it allows the user to zoom into this quadrant - Bottom Left`, async function (){
        let el = await brandPositioningPage.getQuadrant('bottom-left');
        await el.click()
        let expander = await brandPositioningPage.getQuadrantExpander('bottom-left')
        let vertDeviderLocationOne = await (await brandPositioningPage.getVerticalDivider()).getLocation();
        ...
        await expander.click();
        assert.notDeepEqual(vertDeviderLocationOne, vertDeviderLocationtwo, "Clicking on the exander does not cause zoom - Bottom Left")
    })
