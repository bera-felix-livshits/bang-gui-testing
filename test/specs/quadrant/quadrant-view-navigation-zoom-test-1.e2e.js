...
describe(`Quadrant View Navigation - Zoom - Test 1`, () => {
    ...
    it(`Icon should be an arrow before it is expanded (top-left)`, async function () {
        let quad = await brandPositioningPage.getQuadrant(`top-left`);
        await quad.moveTo();
        let baseQuad = await brandPositioningPage.getQuadrantExpander('top-left');

        assert.equal(baseQuad.icon, "expand", "Icon should be an expanding arrow, but is not")
    })

    it(`Verify that the Quadrant zooms in and fills the space (top-left)`, async function () {
        let baseQuad = await brandPositioningPage.getQuadrantExpander('top-left');
    ...
    it(`Icon should be an "X" after it is expanded (top-left)`, async function () {
        // let quad = await brandPositioningPage.getQuadrant(`top-left`);
        // await quad.moveTo();
        let baseQuad = await brandPositioningPage.getQuadrantExpander('top-left');
        // await baseQuad.el.click();
    ...
    it(`Verify that the view pans to that new quadrant when the Top Right quadrant is clicked (top-left to top-right)`, async function (){
        let quad = await brandPositioningPage.getQuadrant(`top-right`);
        await quad.moveTo();
        let topRight = await brandPositioningPage.getQuadrantExpander('top-right');
        await topRight.el.click();
    ...
    it(`Verify that the view pans to that new quadrant when the Top Right quadrant is clicked (top-left to bottom-left)`, async function (){
        let quad = await brandPositioningPage.getQuadrant(`bottom-left`);
        await quad.moveTo();
        let topRight = await brandPositioningPage.getQuadrantExpander('bottom-left');
        await topRight.el.click();
    ...
    it(`Verify that the view pans to that new quadrant when the Top Right quadrant is clicked (top-left to bottom-right)`, async function (){
        let quad = await brandPositioningPage.getQuadrant(`bottom-right`);
        await quad.moveTo();
        let topRight = await brandPositioningPage.getQuadrantExpander('bottom-right');
        await topRight.el.click();
    ...
    it(`Verify that the view is zoomed back out to the normal 2x2 Quadrant View (top-left)`, async function (){
        let baseQuad = await brandPositioningPage.getQuadrantExpander('top-left');
        await baseQuad.el.click()
